...
import DemoLeadDetail from './DemoLeadDetail'
...
function PriorityBadge({ value }: { value: number | null }) {
  ...
  let color = '#5A5E82'
  if (v >= 80) { bg = 'rgba(91,110,245,0.18)'; color = '#A99FF5' }
  else if (v >= 50) { bg = 'rgba(236,178,46,0.12)'; color = '#ECB22E' }
  return (
    <span className="inline-flex items-center justify-center w-8 h-8 rounded-full text-xs font-bold" style={{ background: bg, color }}>
      {v}
    </span>
  )
}
...
export default function DemoLeadTable({ leads }: { leads: Lead[] }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4 items-center">
        ...
        />
        <select value={scoreFilter} onChange={e => setScoreFilter(e.target.value)} style={{ ...inputStyle, appearance: 'auto' }}>
        ...
        <select value={sectorFilter} onChange={e => setSectorFilter(e.target.value)} style={{ ...inputStyle, appearance: 'auto' }}>
          ...
          <table className="w-full text-sm">
            <thead style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <tr style={{ background: 'rgba(255,255,255,0.02)' }}>
                <th className="text-left px-4 py-3 font-medium cursor-pointer select-none w-16" style={{ color: '#5A5E82' }} onClick={() => toggleSort('ai_prioriteit')}>
                  Prio <Arrow k="ai_prioriteit" />
                </th>
                <th className="text-left px-4 py-3 font-medium cursor-pointer select-none" style={{ color: '#5A5E82' }} onClick={() => toggleSort('naam')}>
                  Naam <Arrow k="naam" />
                </th>
                ...
                <th className="text-left px-4 py-3 font-medium cursor-pointer select-none" style={{ color: '#5A5E82' }} onClick={() => toggleSort('aangemaakt_op')}>
      ...
      {selected && <DemoLeadDetail lead={selected} onClose={() => setSelected(null)} />}
    </div>
  )
}
